...
use crate::model::{
    AgentId, AgentRecord, EvidenceId, EvidenceKind, EvidenceQuality, EvidenceRecord,
...
fn report_supports_source_confirmation_kind(report: &ReportRecord) -> bool {
...
fn bind_variant_findings(
        ...
        .filter(|(_, found)| found.variant_count > 1)
    {
        ...
        let (reference, suggested) = if variant_drifted {
            ...
            if !variant_ids.is_empty() && source_confirmation_variant_ids == variant_ids {
...
fn matching_variant_finding_id(
    ...
    matching_report_finding_id(
        store,
        report,
        crate::query::SAME_NAME_DIVERGENT_FINDING_KIND,
        |affected| affected == variant_ids,
    )
}
...
mod recovery_tests {
    ...
    fn scan_classifies_a_non_unicode_identity_path_without_lossy_text() {
        ...
        let envelope: Value = serde_json::from_str(&error_json("scan", &error)).unwrap();
        assert_eq!(
            envelope.pointer("/error/code"),
            Some(&json!("non_unicode_identity_path"))
        );
